import React from "react";
import Card from "react-bootstrap/Card";
import { ImPointRight } from "react-icons/im";

function AboutCard() {
  return (
    <Card className="quote-card-view">
      <Card.Body>
        <blockquote className="blockquote mb-0">
          <p style={{ textAlign: "justify" }}>
            Hi Everyone, I am <span className="purple">Choukri Soukehal </span>
            from <span className="purple"> Quebec city.</span>
            <br />
            I am currently employed as a software engineer at CTRL.
            <br />
            I have completed Bacherlor degree (IMSc) in software engineering in Big data at University of Laval.
            <br />
            <br />
            Apart from Analyzing data and coding, some other activities that I love to do!
          </p>
          <ul>
            <li className="about-activity">
              <ImPointRight /> Analyzing Data (Ok not very funny)
            </li>
            <li className="about-activity">
              <ImPointRight /> Chess
            </li>
            <li className="about-activity">
              <ImPointRight /> Travelling
            </li>
          </ul>

          <p style={{ color: "rgb(155 126 172)" }}>
            "Strive to build things that make a difference!"{" "}
          </p>
          <footer className="blockquote-footer">Soukehal</footer>
        </blockquote>
      </Card.Body>
    </Card>
  );
}

export default AboutCard;
